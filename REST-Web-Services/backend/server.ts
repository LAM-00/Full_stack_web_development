...
// ----------------------------------------
function jsonError(res: any, status: number, error: string, message: string) {
...
app.get("/api/posts", async (req, res) => {
  try {
    ...
    const [totalRows] = await pool.query<RowDataPacket[]>(
      `SELECT COUNT(*) AS total FROM posts ${searchSql}`,
      searchParams
    );
